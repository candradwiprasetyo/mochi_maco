...
function get_jumlah_customer_puas($date1, $date2){
	$query = mysql_query("	
								SELECT count(jumlah) as result
								FROM members a
								JOIN (
								
									SELECT count( transaction_id) AS jumlah, member_id
									FROM transactions b 
									WHERE  b.transaction_date >= '$date1 00:00:00'
									AND b.transaction_date <= '$date2 23:59:59'
								...
								where jumlah > 2
                                order by jumlah desc, a.member_id asc
							 ");
	$result = mysql_fetch_array($query);
...
function get_total_customer(){
	$query = mysql_query("	select count(*) as result from members
							 ");
	$result = mysql_fetch_array($query);
	$result = ($result['result']) ? $result['result'] : 0; 
	return $result;
